...
public class LocationDao
{
     ...
     * @throws 
     */
    public List<Map<String,Object>> queryScatterMapData(String floorNo, int second)
    {
        ...
        String sql = "select a.*,r.role,r.userName,r.phoneNumber from (select * from "
                + "(select * from "
                + tableName
                + " where z = ? and timestamp > ? order by timestamp desc) a group by a.userID) a "
                + " left join register r on a.userID = r.userId";
    ...
    public Collection<LocationModel> getIlsData(String userId)
    {
        long time = System.currentTimeMillis();
        String sql = "select * from locationphone where userId='"+userId+"' and timestamp>"+time+" order by timestamp ";
        // JdbcTemplate tem = this.getJdbcTemplate();
    ...
    /** 
     * @Title: getSvaId 
     * @Description: 根据cgi查询svaId
     * @param cgi
     * @return 
     */
    public Integer getSvaId(int cgi)
    {
        String sql = "select svaId from info_cgi_mapper where cgi = '"+cgi+"' limit 1;";
