...
public class MessageDao
{
    ...
    private class MessageMapper2 implements RowMapper
    {
        public Object mapRow(ResultSet rs, int num) throws SQLException
        {
            MessageModel msg = new MessageModel();
            ...
            msg.setMessageId(rs.getString("ID"));
            msg.setShopId(rs.getInt("SHOPID"));
            msg.setTicketPath(rs.getString("TICKETPATHS"));
            return msg;
        }
    }
    ...
    public Collection<MessageModel> getAllMessageDataByAreaIdNew(String areaId)
    {
        String sql = "SELECT a.*,b.floor,d.ticketPath ticketPaths,c.name place FROM message a left join maps b on a.floorNo = b.floorNo left join store c on a.placeId = c.id left join ticket d on a.id = d.msgId where a.isEnable='开启' and a.shopId="+areaId+" group by a.id";
        // JdbcTemplate tem = this.getJdbcTemplate();
        return this.jdbcTemplate.query(sql, new MessageMapper2());
    }
    //
    ...
    {
        String sql = "select id from message where shopId = ?";
        String[] parem = {areaId};
        return this.jdbcTemplate.queryForList(sql,parem,String.class);
    }
    ...
    {
        String sql = "select ticketPath,chances from ticket where msgId = ?";
        String[] parem = {msgId};
        return this.jdbcTemplate.queryForList(sql,msgId);
    }
}
